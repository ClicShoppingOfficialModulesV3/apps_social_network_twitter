<?php
  ...
  class PageContentTwitter implements \ClicShopping\OM\Modules\HooksInterface
  {
    ...
    public function display()
    {
      ...
      $this->app->loadDefinitions('Module/Hooks/ClicShoppingAdmin/BlogContent/PageContentTwitter');
      ...
      if (CLICSHOPPING_APP_TWITTER_TW_STATUS == 'True' && !empty(CLICSHOPPING_APP_TWITTER_TW_CONSUMMER_KEY)) {
        ...
        $content .= '<div class="col-md-5">';
        $content .= HTML::radioField('blog_status_twitter', '1', $pInfo->in_accept_twitter) . '&nbsp;' . $this->app->getDef('text_yes') . '&nbsp;' . HTML::radioField('blog_status_twitter', '0', $pInfo->out_accept_twitter) . '&nbsp;' . $this->app->getDef('text_no');
        $content .= '</div>';
        $content .= '</div>';
        $content .= '</div>';
        $content .= '</div>';
